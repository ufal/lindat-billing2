...
";
  ...
  print SQL " -- aggregating endpoints data
  ...
  for my $ip (keys %{$aggr_ip_data->{$period}}) {
    for my $token_used (keys %{$aggr_ip_data->{$period}->{$ip}}) {
      for my $service (keys %{$aggr_ip_data->{$period}->{$ip}->{$token_used}}){
        ...
        print $fh sprintf("UPDATE log_ip_aggr SET cnt_requests = cnt_requests + %d, cnt_units = cnt_units + %d, cnt_body_bytes_sent = cnt_body_bytes_sent + %d "
                      ."%s;\n", @{$aggr_ip_data->{$period}->{$ip}->{$token_used}->{$service}}, $restrictions);
        print $fh sprintf("INSERT INTO log_ip_aggr(period_start_date, period_end_date, period_level, ip, service_id, cnt_requests, cnt_units, cnt_body_bytes_sent, token_used)"
                      ."SELECT '%s', '%s', '%s', "
                      ."%s, %s, %d, %d, %d, %s "
